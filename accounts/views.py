...
def signup_doctor(request):
    ...
    if request.method == 'POST':
        required_fields = ['username', 'email', 'name', 'age', 'gender', 'address', 'mobile', 
        ...
        if all(field in request.POST and request.POST[field] for field in required_fields):
            ...
            # Check if doctor is at least 27 years old
            if int(age) < 27:
            ...
            try:
                registration_date = datetime.strptime(year_of_registration, "%d/%m/%Y").date()
            ...
            if password == password1:
                if User.objects.filter(username=username).exists():
                    ...
                    user = User.objects.create_user(username=username, password=password, email=email)
                    user.is_active = False  # Set inactive until approved
                    user.save()

                    doctornew = doctor(
                        user=user, name=name, age=age, gender=gender, address=address, 
